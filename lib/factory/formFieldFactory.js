...
class formFieldFactory extends Component {
    ...
    static build(initialField, key) {
        const errors = validateField(defaultTo(initialField.validators, []), initialField.value);
        const field = Object.assign({}, initialField, {errors});
        let fieldComponent;
        switch (field.type) {
            case 'text':
            ...
            case 'creatable':
                ...
                break;
            case 'multicreatable':
                fieldComponent =  <CreatableMultiSelectField { ... field } />;
                break;
            case 'checkbox':
